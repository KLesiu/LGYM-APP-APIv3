...
The complete focused catalog is internal `IAccountSubscriptionGrantRepository`, `ISubscriptionInboxEventRepository`, `IAccountPaidAccessProjectionRepository`, `IAppleSubscriptionProvider`, and `IGooglePlaySubscriptionProvider`; public `IVerifyAppleSubscriptionPurchaseUseCase.VerifyAsync`, `IVerifyGooglePlaySubscriptionPurchaseUseCase.VerifyAsync`, `IIngestAppleSubscriptionNotificationUseCase.IngestAsync`, `IIngestGooglePlayNotificationUseCase.IngestAsync`, `ICurrentPaidAccessQuery.GetAsync`, `ISubscriptionInboxProcessingUseCase.ProcessBatchAsync`, and `ISubscriptionProviderReconciliationUseCase.ReconcileBatchAsync`. Every identity and method is future and not implemented by #443. Child issues may supersede a name only by updating this canonical contract and its guard in the same change.
...
Worker invokes the public processing and reconciliation batches directly. `CancellationToken` is the only Hangfire batch argument. Identity-owned persistence performs record selection and retains record IDs and cursors; Hangfire receives no record IDs/cursors, credentials, raw or provider payloads, receipts/JWS, purchase tokens, or account-binding tokens.
...
These architecture decisions are future policy unless explicitly identified as an official provider fact. Provider calls use a bounded, implementation-owned timeout and a bounded, implementation-owned retry policy. The timeout/retry leaves and values are deferred to the implementing child issue; calls remain cancellation-aware and retries remain safe/idempotent, bounded, and responsive to `Retry-After` and provider transient guidance without adding runtime configuration or values in #443.

Future Apple calls use the fixed production and sandbox authorities above, TLS 1.2 or later, ES256 Bearer JWT authorization with credential privacy, signed JWS validation, and production current-state/history re-query. Apple documents 429 handling through `Retry-After`; production and sandbox notification retry behavior differs, so environment handling must remain explicit.
...
The provider-private trust zone is the only place that may handle or store receipts/JWS, purchase tokens, account-binding tokens, credentials, or provider bodies. Those sensitive classes are forbidden from public contracts, logs, analytics, API or Worker responses, and test or operational evidence. The sole permitted observability and evidence class is `metadata-only`: stable identifiers, categories, outcomes, timestamps, retry state, and aggregate counts that cannot reconstruct a sensitive value. The complete redaction classes are `signed-payload`, `purchase-token`, `account-binding-token`, `credential`, `provider-body`, `personal-data`, and `metadata-only`; every class except `metadata-only` is excluded from observability and evidence. This document contains no actual receipt/JWS, purchase token, account-binding token, credential, provider body, or personal data.
