...
- `#381` defines Notifications write ownership and provider-neutral contracts. #387 later relocated its approved implementation, persistence, and provider sources into the Notifications assembly.
...
| Runtime root | `LgymApi.Application/ServiceCollectionExtensions.cs` | Application | Application registration composes the remaining feature use cases. | Extracted modules register through their public facades. |
| Runtime root | `LgymApi.Infrastructure/ServiceCollectionExtensions.cs` | Infrastructure | Infrastructure registration owns the shared technical wiring, context bridges, UoW, migrations, and remaining adapters. | Extracted modules keep their own services, repositories, and providers. |
...
| Push contract path | `LgymApi.Notifications/Contracts/Push/` | `Notifications` | Push payloads, outcomes, scheduler, and provider ports describe notification delivery. | Worker selects and implements scheduling, Notifications implements FCM, and Common retains only the persisted push job interface. |
...
| Persistence port | `IReportPhotoPersistence / ReportPhotoPersistenceRepository` | `Reporting` | Photo metadata and upload-session state belong to the Reporting evidence lifecycle. | Non-owners use the Reporting photo workflow only. |
| Persistence port | `IReportingRelationshipAccessPersistence / ReportingRelationshipAccessPersistenceRepository` | `Reporting` | Reporting consumes active relationship facts through marker-safe account IDs. | Identity and Coaching entities or repositories do not cross this port. |
...
| Email registration | `AddNotificationsModule(configuration)` and its private email helper | `Notifications` | Notifications owns email option validation, composer registrations, and Dummy or SMTP sender selection. | Infrastructure retains only the email-lease settings bridge; Platform does not register email descriptors; Worker and Common keep runtime and wire responsibilities. |
...
- The Notifications rows define module/write responsibility. #387 moved approved Notifications implementation, persistence, and provider sources into `LgymApi.Notifications`; entities remain in `LgymApi.Domain/Entities`, and no separate persistence root exists.
...
- Application, Identity, and Notifications business services use direct constructor injection. High constructor arity has no numeric cap when it keeps focused collaborators explicit. Dependency aggregates and service-locator wrappers are prohibited and must not be restored.
