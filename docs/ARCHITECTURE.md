...
The eight owners and 48 entities above are current executable facts. The proposed direct-store subscription boundary does not add an owner or a persisted entity: Identity & Accounts would own its provider-private business rules, logical writes, and adapters, while Infrastructure would retain the single physical persistence, Unit of Work, migration, and Hangfire roots. No subscription-specific context, schema, migration stream, or Hangfire root is proposed. See [Direct Store Subscriptions](subscriptions/direct-store-subscriptions.md) for the detailed future-only contract.
...
- The proposed subscription boundary likewise makes server-side durable current access the authority for paid access. A client success response, unverified provider notification, or long-lived paid JWT claim, role, or permission cannot grant it. The free baseline remains unchanged and is not a fourth paid tier; independent Apple and Google grants would project the highest currently valid tier without automatically cancelling or refunding the other store.
...
- The future direct-store subscription contract is covered by documentation and architecture guards only until a separately approved implementation exists. Those checks preserve the current owner, graph, shared-persistence, and Common authorities; they do not claim provider calls, cryptography, configuration binding, or paid-benefit enforcement.
...
For the proposed direct-store subscription boundary, API would remain thin authenticated or provider-ingress transport to focused Identity contracts, and Worker would directly schedule Identity public use cases through the existing Worker-to-Identity edge. Infrastructure retains the shared Hangfire composition, while `LgymApi.BackgroundWorker.Common` remains closed: no subscription job, payload, provider, or scheduling type belongs there. The future `Subscriptions:*` launch-control roots fail closed and name no current options, values, registrations, or capability enforcement.
...
- [Direct Store Subscriptions](subscriptions/direct-store-subscriptions.md) is the canonical future-only contract for issue #443. Its [approved V0 store catalog](subscriptions/store-catalog.md) records commercial intent as `approved-precreation`, not runtime implementation or production enablement. The boundary preserves the current topology while reserving Identity provider-private ownership, API transport, Worker scheduling, Infrastructure technical roots, Common closure, server current-access authority, launch controls, and the independent cross-store highest-valid-tier projection.
...
The future subscription contract adds no project-reference edge or cycle, persistence change, runtime implementation, provider, endpoint, job, option, or paid enforcement. The linked approved-precreation catalog changes none of those boundaries. Its logical flow uses the existing API-to-Identity and Worker-to-Identity paths only.
