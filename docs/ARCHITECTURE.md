...
- [Direct Store Subscriptions](subscriptions/direct-store-subscriptions.md) is the canonical future-only contract for issue #443. It preserves the current topology while reserving Identity provider-private ownership, API transport, Worker scheduling, Infrastructure technical roots, Common closure, server current-access authority, launch controls, and the independent cross-store highest-valid-tier projection.
...
The future subscription contract adds no project-reference edge or cycle, persistence change, runtime implementation, provider, endpoint, job, option, or paid enforcement. Its logical flow uses the existing API-to-Identity and Worker-to-Identity paths only.
