...
internal sealed class PushNotificationService : IPushNotificationService
{
    ...
    private string? Schedule(PushNotificationMessage message)
    {
        ...
        {
            return _pushBackgroundScheduler.ScheduleRetry(message.Id.ToString(), nextAttemptAt - DateTimeOffset.UtcNow);
        }

        return _pushBackgroundScheduler.Enqueue(message.Id.ToString());
    }
