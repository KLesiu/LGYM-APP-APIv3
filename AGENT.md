...
| `LgymApi.E2ETests/LgymApi.E2ETests.csproj` | Standalone package-only E2E harness for configuration, runner policy, boundary, fresh PostgreSQL, and external published-API/Expo/browser lifecycle proofs through public HTTP. It has zero direct/evaluated project references and is not one of the main solution's 18 projects or 90 direct edges. | Keep it outside `LgymApi.sln`; use only package dependencies and public HTTP. Approved E2E browser lifecycle probes are allowed, but product business scenarios remain deferred. Keep private failure artifacts ignored, sanitized, and never committed. |
