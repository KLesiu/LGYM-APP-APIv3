...
/// </summary>
internal sealed class TrainingMainRecordsUpdateService : Contracts.BackgroundActions.ITrainingMainRecordsUpdatePort
{
    private readonly IMainRecordRepository _mainRecordRepository;
    private readonly TrainingMainRecordsData _trainingData;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IUnitConverter<WeightUnits> _weightUnitConverter;
    private readonly ILogger<TrainingMainRecordsUpdateService> _logger;

    public TrainingMainRecordsUpdateService(
        IMainRecordRepository mainRecordRepository,
        TrainingMainRecordsData trainingData,
        IUnitOfWork unitOfWork,
        IUnitConverter<WeightUnits> weightUnitConverter,
        ILogger<TrainingMainRecordsUpdateService> logger)
    {
    ...
    public async Task UpdateAsync(string payloadJson, CancellationToken cancellationToken = default)
    {
        ...
        var training = await _trainingData.Trainings.GetByIdAsync((Domain.ValueObjects.Id<Domain.Entities.Training>)command.TrainingId, cancellationToken);
        if (training is null)
        {
            _logger.LogWarning(
        ...
        var trainingExerciseScores = await _trainingData.TrainingExerciseScores.GetByTrainingIdsAsync(
        ...
        var exerciseScores = await _trainingData.ExerciseScores.GetByIdsAsync(exerciseScoreIds, cancellationToken);
...
internal sealed class TrainingMainRecordsData(
